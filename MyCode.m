...
%Using the matrix to calculate statistical data
for n = 1:length(dataFromColumns(1,:))
   MeanOfColumns(n) = mean(dataFromColumns(:,n))
   MedianOfColumns(n) = median(dataFromColumns(:,n))
   StandardDeviarionOfColumns(n) = std(dataFromColumns(:,n))
end 
